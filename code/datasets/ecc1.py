...
#DATA_PATH_V1 = "/home/qiu/3drepresentation/votenet-main/sunrgbd1/sunrgbd_pc_bbox_votes_50k_v1" ## Replace with path to dataset
#DATA_PATH_V1= "/home/qiu/change_caption/dataset/ecc_v0"
DATA_PATH_V1 = "/groups/gaa50131/datasets/pcdre/pcdre" 
DATA_PATH_V2 = "" ## Not used in the codebase.


class ECC1DatasetConfig(object):
    def __init__(self):
...
class ECC1DetectionDataset(Dataset):
    def __init__(
        self,
        dataset_config,
        split_set="train",
    ...
    def __getitem__(self, idx):
        ...
        point_cloud = np.load(scan_path + "_pcd.npz")["pc"]  # Nx6
        bboxes = (np.load(scan_path + "_bbox.npy")[:,0:8])  # K,7
        #bboxes_1 = (np.load(scan_path + "_bbox.npy")[:,0:7])  # K,8
        #obj_classes = (np.load(scan_path + "_bbox.npy")[:,7:8])  # K,1 ###----- Object Classes ----###TODO###
        allcaps = np.load(scan_path + "_question.npz")["arr_0"] ##TODO### #####----------------------------------------------------------------------------------------------------------##### 
        allres = np.load(scan_path + "_captions.npz")["arr_0"]
        #print(scan_name)
        ...
        max_bboxes = np.zeros((self.max_num_obj, 7))
        max_bboxes[0 : bboxes.shape[0], :] = bboxes[:,0:7]  ########################### 
        ...
        ###----------------------------------------###TODO               
        target_bboxes_objsemcls = np.zeros((self.max_num_obj)) ##TODO##
        target_bboxes_objsemcls[0 : bboxes.shape[0]] = bboxes[:,-1] ##TODO##
        #print('???')
        #print(obj_classes)  
        #print('dddd')
        ...
        res = np.zeros((1, 29))
        re_lens = np.zeros((1))
        captions = np.zeros((1, 29)) ##TODO
        caption_lens = np.zeros((1)) ##TODO

        #gt_bboxes = np.zeros((self.max_num_obj, 3))
        #gt_bboxes[0 : bboxes.shape[0]] = bboxes[:,0:3]
        #gt_bboxes[0 : bboxes.shape[0],0] = bboxes_1[:,1]
        ...
        #key_ids = 0
        #for key in allres:
        #curr_cap_id = random.randint(0,len(allres[key])-1)
          #print(allcaps[key][curr_cap_id])                                                                                                                                                                                                                                      
        res[0] = allres[1:]
        re_lens[0] = allres[0]
        #key_ids = key_ids + 1
        ...
        ###----------------------------------------###TODO       
        ret_dict["gt_box_sem_cls_label"] = target_bboxes_objsemcls.astype(np.int64) ##TODO##
        ret_dict["gt_captions"] = captions.astype(np.int64) ##TODO##
        ret_dict["gt_caption_lens"] = caption_lens.astype(np.int64) ##TODO##
        ret_dict["res"] = res.astype(np.int64) ##TODO##                                                                                                                                                                                                             
        ret_dict["re_lens"] = re_lens.astype(np.int64) ##TODO##    

        #ret_dict["gt_bboxes_forpbb"] = gt_bboxes.astype(np.float32)
        ###----------------------------------------###TODO       
        
        ret_dict["gt_box_present"] = target_bboxes_mask.astype(np.float32)
        ret_dict["scan_idx"] = np.array(idx).astype(np.int64)
        #ret_dict["scan_idx1"] = scan_name
