...
DATA_PATH_V1 = "/groups/gab50221/datasets/scene_change_captioning/eccv1_per90/eccv1_per90" 
...
class ECCDatasetConfig(object):
    def __init__(self):
        ...
        #self.num_semcls = 8  ####
        self.num_objsemcls = 18 ##TODO##
        self.num_angle_bin = 12   ####
        self.max_num_obj = 12  ####################TODO######################
        self.vocab_size = 81 ##TODO##
...
class ECCDetectionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        bboxes = (np.load(scan_path + "_bbox.npy")[:,0:7])  # K,7
        #bboxes_1 = (np.load(scan_path + "_bbox.npy")[:,0:3])  # K,8
        obj_classes = (np.load(scan_path + "_bbox.npy")[:,7:8])  # K,1 ###----- Object Classes ----###TODO###
        allcaps = (np.load(scan_path + "_captions.npz",allow_pickle=True)) ##TODO### #####----------------------------------------------------------------------------------------------------------##### 
        ...
        target_bboxes_objsemcls[0 : bboxes.shape[0]] = obj_classes[:,0] ##TODO##
  
        captions = np.zeros((self.max_num_obj, 23)) ##TODO
        caption_lens = np.zeros((self.max_num_obj)) ##TODO
        gt_bboxes = np.zeros((self.max_num_obj, 3))
        gt_bboxes[0 : bboxes.shape[0]] = bboxes_1[:,0:3]
        ...
        ###----------------------------------------###TODO       
        ret_dict["gt_box_obj_sem_cls_label"] = target_bboxes_objsemcls.astype(np.int64) ##TODO##
